...
export default function App() {
  const [page, setPage] = useState("dashboard"); const [selected, setSelected] = useState("Al"); const [compare, setCompare] = useState(["Al", "Fe", "Cu", "Ti"]); const [session, setSession] = useState(null);
  const pages = useMemo(() => ({ dashboard: <Dashboard setPage={setPage}/>, login: <LoginAccount setPage={setPage}/>, explorer: <Explorer selected={selected} setSelected={setSelected} setCompare={setCompare}/>, periodic: <PeriodicTable selected={selected} setSelected={setSelected}/>, compare: <Compare compare={compare} setCompare={setCompare} setPage={setPage}/>, atlas: <BehaviourAtlas selected={selected} setSelected={setSelected}/>, graph: <BehaviourGraph selected={selected} setSelected={setSelected}/>, universe: <SimilarityUniverse selected={selected} setSelected={setSelected}/>, calculations: <CalculationCore/>, reports: <Reports compare={compare}/> }), [page, selected, compare]);
  return <div className="min-h-screen bg-[#020617] text-slate-100"><Background/><Sidebar page={page} setPage={setPage}/><main className="relative z-10 space-y-6 p-4 lg:ml-[310px] lg:p-8"><div className="flex flex-wrap items-center justify-between gap-3 lg:hidden"><div><div className="text-2xl font-black tracking-[.18em] text-cyan-100">ElementOS</div><div className="text-xs uppercase tracking-[.22em] text-slate-500">material intelligence</div></div><select value={page} onChange={(e) => setPage(e.target.value)} className="rounded-2xl border border-white/10 bg-slate-950 p-3 outline-none">{Object.keys(pages).map(k => <option key={k} value={k}>{k}</option>)}</select></div>{pages[page]}</main></div>;
}
