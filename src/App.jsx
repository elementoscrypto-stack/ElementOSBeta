...
function makeExportSvg({ title = "ElementOS Export", summary = "", payload = {}, sections = [] }) {
  const story = [summary, ...sections.map((section) => `${section.label}: ${section.value}`)].filter(Boolean).join(" • ");
  const lines = wrapExportLines(story || JSON.stringify(payload, null, 2), 58, 9);
  const metricEntries = Object.entries(payload || {})
    .filter(([, value]) => ["string", "number", "boolean"].includes(typeof value))
    .slice(0, 6);
  const metricTiles = metricEntries.map(([key, value], index) => {
    const x = 80 + (index % 3) * 300;
    const y = 720 + Math.floor(index / 3) * 150;
    return `<rect x="${x}" y="${y}" width="260" height="110" rx="28" fill="rgba(255,255,255,.055)" stroke="rgba(34,211,238,.28)"/>\n<text x="${x + 24}" y="${y + 42}" fill="#7dd3fc" font-family="Inter, Arial" font-size="18" font-weight="900" letter-spacing="2">${escapeXml(String(key).toUpperCase().slice(0, 22))}</text>\n<text x="${x + 24}" y="${y + 82}" fill="#ffffff" font-family="Inter, Arial" font-size="28" font-weight="950">${escapeXml(String(value).slice(0, 20))}</text>`;
  }).join("\n");
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1350" viewBox="0 0 1080 1350">
  <defs>
    <linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#020617"/><stop offset=".55" stop-color="#071426"/><stop offset="1" stop-color="#082f49"/></linearGradient>
    <radialGradient id="orb" cx="50%" cy="12%" r="70%"><stop offset="0" stop-color="#22d3ee" stop-opacity=".32"/><stop offset="1" stop-color="#020617" stop-opacity="0"/></radialGradient>
  ...
  <g opacity=".24">${Array.from({length: 22}).map((_,i)=>`<line x1="0" y1="${i*64}" x2="1080" y2="${i*64}" stroke="#38bdf8" stroke-width="1"/>`).join("")} ${Array.from({length: 18}).map((_,i)=>`<line x1="${i*64}" y1="0" x2="${i*64}" y2="1350" stroke="#38bdf8" stroke-width="1"/>`).join("")}</g>
  <circle cx="880" cy="170" r="190" fill="#22d3ee" opacity=".12" filter="url(#glow)"/>
  <circle cx="210" cy="1140" r="240" fill="#f59e0b" opacity=".10" filter="url(#glow)"/>
  <rect x="58" y="58" width="964" height="1234" rx="54" fill="rgba(2,6,23,.52)" stroke="rgba(125,211,252,.34)" stroke-width="2"/>
  <text x="80" y="128" fill="#fbbf24" font-family="Inter, Arial" font-size="20" font-weight="950" letter-spacing="5">ELEMENTOS EXPORT</text>
  <text x="80" y="218" fill="#ffffff" font-family="Inter, Arial" font-size="62" font-weight="950">${escapeXml(String(title).slice(0, 30))}</text>
  <text x="80" y="278" fill="#67e8f9" font-family="Inter, Arial" font-size="30" font-weight="900">PDF · JSON · SVG</text>
  <rect x="80" y="340" width="920" height="310" rx="34" fill="rgba(15,23,42,.70)" stroke="rgba(34,211,238,.28)"/>
  ${lines.map((line, index) => `<text x="116" y="405" fill="#dbeafe" font-family="Inter, Arial" font-size="28" font-weight="700">${escapeXml(line)}</text>`.replace('y="405"', `y="${405 + index * 39}"`)).join("\n")}
  ${metricTiles}
  <rect x="80" y="1116" width="920" height="112" rx="34" fill="rgba(34,211,238,.10)" stroke="rgba(34,211,238,.30)"/>
  <text x="116" y="1163" fill="#67e8f9" font-family="Inter, Arial" font-size="22" font-weight="950" letter-spacing="3">DISCOVER · SIMULATE · UNDERSTAND · SHARE</text>
  <text x="116" y="1204" fill="#ffffff" font-family="Inter, Arial" font-size="26" font-weight="900">Generated ${escapeXml(new Date().toLocaleString())}</text>
  <text x="80" y="1266" fill="#64748b" font-family="Inter, Arial" font-size="18" font-weight="800">ElementOS universal export bundle</text>
</svg>`;
}
...
function exportAllFormats({ baseName = "elementos-export", title = "ElementOS Export", summary = "", payload = {}, sections = [], customSvg = null }) {
  ...
  downloadFile(`${slug}.svg`, customSvg || makeExportSvg({ title, summary, payload: normalizedPayload, sections }), "image/svg+xml");
}
